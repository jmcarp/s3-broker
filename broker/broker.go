...
func (b *S3Broker) getBucketNames(instanceNames []string, instanceGUID, planGUID string) ([]string, error) {
	bucketNames := make([]string, len(instanceNames))
...
func (b *S3Broker) Bind(
	...
	bucketNames, err := b.getBucketNames(bindParameters.AdditionalInstances, instanceID, details.PlanID)
	if err != nil {
		return binding, err
	}
	...
	for idx, bucketName := range bucketNames {
		...
		case bucketDetails := <-detailc:
			...
			if bucketDetails.BucketName == bucketName {
